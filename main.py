...
Supports:
- Existing movie mode: one or many BMS movie URLs.
- Theatre mode: give only a theatre name and monitor any movie playing there.
"""
    ...
    for show in shows:
        ...
        if periods:
            try:
                ...
                time_code = 0
            if not any(period in TIME_PERIODS and TIME_PERIODS[period][0] <= time_code < TIME_PERIODS[period][1] for period in periods):
                continue
        result.append(show)
    return result
...
def detect_changes(old_state, new_state):
    ...
    for key in set(new_shows) - set(old_shows):
        show = new_shows[key]
        changes.append(f"🆕 NEW: {show['venue']} {show['time']} [{show['date']}] — {show['cat']} ₹{show['price']}")
    for key, current in new_shows.items():
        ...
        if previous and previous["status"] == "0" and current["status"] != "0":
            ...
            changes.append(f"{icon} BACK: {current['venue']} {current['time']} [{current['date']}] — {current['cat']} → {label}")
    return changes
...
def send_email(subject, changes, shows, movie_info):
    ...
    change_html = ""
    if changes:
        rows = "".join(f'<li style="padding:3px 0;font-size:14px;">{escape(item)}</li>' for item in changes)
        change_html = f'<h3>Changes Detected</h3><ul style="line-height:1.6;">{rows}</ul>'

    shows_html = ""
    for venue_name, venue_shows in venue_groups.items():
        rows = ""
        for show in venue_shows:
            cats = " | ".join(f"{escape(cat.name)} Rs.{escape(cat.price)} ({status_label(cat.status)})" for cat in show.categories)
            screen = f" [{escape(show.screen_attr)}]" if show.screen_attr else ""
            rows += f'<tr><td style="padding:5px 8px;border-bottom:1px solid #ddd;">{escape(show.time)}{screen}</td><td style="padding:5px 8px;border-bottom:1px solid #ddd;">{cats}</td></tr>'
        shows_html += f'<p><b>{escape(venue_name)}</b></p><table style="width:100%;border-collapse:collapse;"><tr><th style="text-align:left;padding:5px 8px;">Time</th><th style="text-align:left;padding:5px 8px;">Categories</th></tr>{rows}</table>'

    html = f'''<!doctype html><html><body style="font-family:Arial,sans-serif;color:#333;padding:24px;">
<h2 style="color:#111;">BMS Alert: {escape(movie_name)}</h2><p style="color:#666;">{escape(now_str)}</p><hr>
{change_html}<h3>Current Showtimes</h3>{shows_html}
<p style="color:#999;font-size:12px;margin-top:24px;">Automated alert from BMS Ticket Notifier.</p></body></html>'''

    plain = [subject, "", f"Checked at: {now_str}", ""]
    if changes:
        plain += ["Changes Detected:"] + [f"  - {x}" for x in changes] + [""]
    plain.append("Current Showtimes:")
    for venue_name, venue_shows in venue_groups.items():
        plain.append(venue_name)
        for show in venue_shows:
            cats = " | ".join(f"{cat.name} Rs.{cat.price} ({status_label(cat.status)})" for cat in show.categories)
    ...
    try:
        resp = requests.post(
            ...
            json={"from": sender, "to": [to], "subject": subject, "text": "\n".join(plain), "html": html},
            timeout=15,
        )
...
def discover_theatre_page(region_slug, theatre_filter):
    """Find a BMS cinema page from the public cinema directory."""
    directory = f"https://in.bookmyshow.com/{region_slug}/cinemas"
    try:
        resp = requests.get(directory, headers=HEADERS, timeout=20)
        if resp.status_code != 200:
            print(f"  ❌ Cinema directory HTTP {resp.status_code}")
            return None
        html = resp.text
    except requests.RequestException as exc:
        print(f"  ❌ Cinema directory request failed: {exc}")
        return None

    target = theatre_filter.lower().strip()
    # Prefer anchors whose visible text matches the requested theatre.
    anchor_re = re.compile(r'<a\b[^>]*href=["\']([^"\']*?/cinemas/[^"\']+)["\'][^>]*>(.*?)</a>', re.I | re.S)
    candidates = []
    for href, body in anchor_re.findall(html):
        text = clean_html_text(body)
        full_href = urljoin(directory, href)
        if "/cinemas/" not in full_href:
            continue
        if target and target in text.lower():
            candidates.append((text, full_href))

    if not candidates:
        # Fallback: match the theatre name against the URL slug.
        slug_target = re.sub(r"[^a-z0-9]+", "-", target.lower()).strip("-")
        for href in re.findall(r'href=["\']([^"\']*/cinemas/[^"\']+)["\']', html, re.I):
            full_href = urljoin(directory, href)
            slug = full_href.lower()
            if slug_target and slug_target in slug:
                candidates.append((full_href, full_href))
...
def discover_event_codes_from_theatre_page(theatre_base_url, date_code):
    """Read event codes from a BMS cinema page for the selected date."""
    url = theatre_base_url.rstrip("/") + (f"/{date_code}" if date_code else "")
    try:
        resp = requests.get(url, headers=HEADERS, timeout=20)
        if resp.status_code != 200:
            print(f"  ❌ Theatre page HTTP {resp.status_code}: {url}")
            return []
        html = resp.text
    except requests.RequestException as exc:
        print(f"  ❌ Theatre page request failed: {exc}")
        return []

    events = []
    # Movie booking links on cinema pages contain /movies/ and an ET event code.
    for href in re.findall(r'href=["\']([^"\']*?/movies/[^"\']*?ET\d{8,}[^"\']*)["\']', html, re.I):
        for code in re.findall(r"ET\d{8,}", href, re.I):
            code = code.upper()
            if code not in events:
                events.append(code)
    # Some BMS renders put event codes directly in embedded JSON.
    if not events:
...
def theatre_dates():
    ...
    # One date is the efficient default. Set BMS_DATES explicitly to monitor more dates.
    return [now.strftime("%Y%m%d")]


def movie_name_matches(movie_name):
    wanted = CONFIG["movie"].strip()
    if not wanted or wanted.upper() == "ANY" or wanted == "*":
        return True
    return wanted.lower() in movie_name.lower()
...
def run_movie(event_code, region_slug, date_list, previous_state, label=""):
    ...
    if movie_info["name"] == "Unknown Movie" and not all_shows:
        return None
    if not movie_name_matches(movie_info["name"]):
        return None
    filtered = filter_shows(all_shows, CONFIG["theatre"], CONFIG["time_period"], CONFIG["dates"])
    new_state = build_state(filtered, all_dates)
    changes = detect_changes(previous_state or {}, new_state)
    ...
    if changes:
        ...
        send_email(f"BMS Alert: {prefix}{movie_info['name']} - {len(changes)} change(s)", changes, filtered, movie_info)
    else:
...
def main_movie_mode(old_movies):
    ...
    for index, url in enumerate(urls, start=1):
        ...
        event_code, region_slug, url_date = parsed["event_code"], parsed["region_slug"], parsed.get("date_code", "")
        print(f"\n🎬 Movie {index}/{len(urls)}")
        if not event_code or not region_slug:
            print("  ❌ Invalid BMS URL — skipping.")
            continue
        if CONFIG["dates"]:
            date_list = [d.strip() for d in CONFIG["dates"].split(",") if d.strip()]
        elif url_date:
            date_list = [url_date]
        else:
            date_list = [""]
...
def main_theatre_mode(old_movies):
    ...
    if not theatre:
        print("  ❌ BMS_THEATRE is required in theatre mode.")
        return {}, False

    region_slug = CONFIG["region"]
    region_slug = "bengaluru" if region_slug == "bangalore" else region_slug
    print(f"\n🏢 Theatre mode — {theatre} | Region: {region_slug} | Movie: {CONFIG['movie']}")
    theatre_info = discover_theatre_page(region_slug, theatre)
    if not theatre_info:
        print("  ❌ Could not find that theatre on BookMyShow.")
        return {}, False
    ...
    any_ok = False
    event_codes = []
    for date_code in theatre_dates():
        found = discover_event_codes_from_theatre_page(theatre_info["url"], date_code)
        for code in found:
            if code not in event_codes:
                event_codes.append(code)
    if not event_codes:
        print("  ⚠️ No movie event codes found on the theatre page.")
        return {}, False
    ...
    for index, event_code in enumerate(event_codes, start=1):
        print(f"\n  Movie {index}/{len(event_codes)} — {event_code}")
        previous = old_movies.get(event_code)
        state = run_movie(event_code, region_slug, date_list, previous, theatre)
        if state is not None:
            new_movies[event_code] = state
            any_ok = True
    return new_movies, any_ok
...
def main():
    ...
    if CONFIG["mode"] in ("theatre", "cinema"):
        new_movies, any_ok = main_theatre_mode(old_movies)
    else:
        new_movies, any_ok = main_movie_mode(old_movies)

    if not any_ok:
        print("\n  ❌ No usable movie/showtime data was produced.")
        return

    save_state({"version": 3, "movies": new_movies})
